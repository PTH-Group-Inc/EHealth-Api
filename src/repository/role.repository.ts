...
export class RoleRepository {
  async findById(id: string, includeDeleted = false) {
    const sql = includeDeleted
      ...
      : `SELECT * FROM app_roles WHERE id=$1 AND deleted_at IS NULL`;
  ...
  async list(params?: {
    ...
    const countSql = `SELECT COUNT(*)::int AS total FROM app_roles ${whereSql};`;
  ...
  async replaceRolePermissions(roleId: string, permissionIds: string[]) {
    ...
    try {
      ...
      if (permissionIds.length > 0) {
        const { rows } = await pool.query(
          `SELECT id FROM app_permissions WHERE id = ANY($1::uuid[]) AND deleted_at IS NULL;`,
          [permissionIds],
        );
        if (rows.length !== permissionIds.length)
          throw new HttpError(400, "Some permissionIds not found or deleted");
      }

      await pool.query(`DELETE FROM app_role_permissions WHERE role_id=$1`, [
        roleId,
      ]);
