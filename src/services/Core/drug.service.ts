...
import { DrugRepository } from '../../repository/Core/drug.repository';
    ...
    static async importDrugs(buffer: Buffer): Promise<any> {
        ...
        for (const row of data) {
            try {
                ...
                if (!categoryId) {
                    throw new Error(`Không tìm thấy Mã Nhóm Thuốc: ${row.category_code}`);
                }
