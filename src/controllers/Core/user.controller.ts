...
export class UserController {
    ...
    /**
     * Vô hiệu hóa nhiều người dùng cùng lúc (Bulk Soft Delete)
     */
    static bulkDeleteUsers = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
        ...
        if (!userIds || !Array.isArray(userIds) || userIds.length === 0) {
            return res.status(400).json({
                success: false,
                code: 'INVALID_INPUT',
                message: 'Danh sách ID người dùng (userIds) không hợp lệ hoặc trống.'
            });
        }
        ...
        await UserService.bulkDeleteUsers(userIds, adminId, ipAddress, userAgent);

        return res.status(200).json({
            success: true,
            message: `Đã vô hiệu hóa thành công ${userIds.length} người dùng.`
        });
    });
    ...
    static getUserRoles = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
        ...
        return res.status(200).json({
            success: true,
            message: "Lấy vai trò hiệu lực thành công",
            data: roles
        });
    });
