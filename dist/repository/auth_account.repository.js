...
class AccountRepository {
    ...
    static async findByEmail(email) {
      ...
      LIMIT 1;
    `;
        const result = await postgresdb_1.pool.query(query, [email]);
    ...
    static async findByPhone(phone) {
      ...
      LIMIT 1;
    `;
        const result = await postgresdb_1.pool.query(query, [phone]);
        return result.rows[0] ?? null;
    }
    /**
    ...
    static async updatePassword(userId, hashedPassword) {
      ...
      WHERE users_id = $2
    `;
        await postgresdb_1.pool.query(query, [hashedPassword, userId]);
    }
    /**
     * Tạo tài khoản mới
     */
    static async createAccountWithProfileAndRole(user, profileId, fullName, roleCode) {
     ...
     * Khóa tài khoản
     */
    static async lockAccount(userId, lockedUntil) {
        await postgresdb_1.pool.query(`
      UPDATE users
      SET locked_until = $1,
